...
function UpdateCategory({oldData, close,closeEditCategory}) {
    ...
    const [img,setimg]=useState(`https://kayalalshbab-api-node.vercel.app/${oldData.image}`);
    ...
    const onSubmit = (values) => {
        ...
        axios.post(`https://kayalalshbab-api-node.vercel.app/controlboard/updateCategory/${oldData._id}`,form).then((res)=>{
            if(res.data.success){
        ...
        <div className="flex flex-col gap-4  bg-white py-10  shadow-xl w-full md:w-[50%] fixed right-0 top-0 z-50 h-full">
                ...
                <form className="flex flex-col gap-5 w-full px-6 my-10" onSubmit={handleSubmit(onSubmit)} >
                    ...
                    <div className="flex flex-col justify-between gap-3">
                        ...
                        <label  className={`overflow-hidden backdrop-grayscale bg-white/30 backdrop-blur-sm flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer ${errors?.image?"border-red-300":"border-gray-300"}`}>
